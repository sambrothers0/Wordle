...
public class WordleFrame {
	...
	private JMenuBar createMenuBar() {
		...
		kidsItem.addActionListener(event -> updateKids());
		difficultyMenu.add(kidsItem);
		
		JMenuItem normalItem = new JMenuItem("Normal");
		normalItem.addActionListener(event -> updateNormal());
		difficultyMenu.add(normalItem);
		
		JMenuItem hardItem = new JMenuItem("Hard");
		hardItem.addActionListener(event -> updateHard());
	...
	public void updateHint(String mode) {
		keyboardPanel.updateHint(mode);
	}
	
	public void updateKids() {
		if (model.getMode() == "kids") {
			return;
		}
		frame.dispose();
		model.difficultyKids();
	...
	public void updateNormal() {
		...
		frame.dispose();
		model.difficultyNormal();
		new WordleFrame(model);
		repaintWordleGridPanel();
	}
	
	public void updateHard() {
		if (model.getMode() == "hard") {
			return;
		}
		frame.dispose();
		model.difficultyHard();
		new WordleFrame(model);
		repaintWordleGridPanel();
	}
